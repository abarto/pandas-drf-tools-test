$(function() {
    ...
    var tip = d3.tip()
        .attr('class', 'd3-tip').html(function(d) {
            return "<div><span class=\"tooltip-title\">" + d.CTYNAME + "</span><br/>" +
    ...
    d3.json("/api/counties/?state=" + stateFipsCode, function(error, data) {
        if (error) throw error;

        data = _.sortBy(data.records, 'TOT_POP').reverse().slice(0, 10);

        x.domain(data.map(function(d) { return d.CTYNAME; }));
        ...
        g.selectAll(".bar")
            ...
            .enter().append("rect")
                .attr("class", "bar")
                .attr("x", function(d) { return x(d.CTYNAME); })
